...
//  const CounselingCard = fetch('https://counseling-api.onrender.com/api/counseling-cards')
//   .then(res => res.json())
...
const CounselingCards: React.FC = () => {
  const [cards, setCards] = useState<CounselingCardType[]>([]);

  useEffect(() => {
    const fetchCards = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/counseling-cards');
