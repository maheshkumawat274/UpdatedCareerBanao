...
function ExamCard({ data, isLoading }: ExamCardType) {
  ...
  // Debugging outputs
  console.log("Raw Last_date:", lastDate); // Debugging log
  console.log("Parsed Last_date:", parsedLastDate); // Debugging log
  console.log("Is Last_date Valid:", parsedLastDate >= today); // Debugging log

  // Check if the parsed last date is valid and not in the past
  const isLastDateValid =
  //Agar chahte ho ki past dates ke cards bhi render ho
    lastDate !== "No Last Date Available" &&
    !isNaN(parsedLastDate.getTime())

    //ye tbhi kaam kregi jb past dates ko nhi show krana chahte
  ...
  if (!isLastDateValid) {
    console.log("Last Date is not valid or is in the past"); // Debugging log
    return (
      <div className="layout-container-right pb-5 flex flex-col gap-6 w-full min-h-[50vh] lg:w-[78%]">
        <img
          src={no_data_found}
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.3 }}
      className="relative flex flex-col p-6 rounded-lg shadow-md border  hover:shadow-lg transition-transform transform hover:scale-105 mb-5 font-poppins bg-white"
    >
      {/* Last Date Badge */}
      <div
        className="absolute top-3 right-3 px-2 py-1 rounded-md text-xs font-bold shadow-md bg-purple-700 text-white"
      >
        Last Date: {lastDate}
      </div>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-1 mt-1">
            <BuildingOffice2Icon className="h-4 w-4 text-purple-700" />
            <span className="inline-block text-xs font-medium text-white bg-purple-700 rounded-full px-2 py-0.5">
              {collegeCategory}
            </span>
          </div>
        </div>
